...
# The third arm starts beside the survivor and ends alone, which is neither
# of the other two labels. Saying so is the difference between a number that
# can be explained later and one that cannot.
third_overlay="$log_dir/host-share-mixed.env"
printf 'export BENCH_HOST_SHARE=mixed\n' > "$third_overlay"
run_arm_mixed() {
  pitlane --env-extra "$2" --env-extra "$third_overlay" run \
    --arms "$1" \
    --questions "batch${N}" \
    --count "$N" \
    --reps 1 \
...
run_arm_mixed "$THIRD" "$free_env" "launch-$THIRD" & pid_c=$!
