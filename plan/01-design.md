...
| `lmcache_manager` | Serves `--l1-size-gb` from `BENCH_LMCACHE_L1_GB`, which the preflight RAM floor is derived from (`+ BENCH_RAM_MARGIN_GB`) rather than hardcoded alongside it — the floor exists because of the pool, so lowering one must lower the other. **Runs L1-only by default**: `LMCACHE_L2_DIR` is blank, `--l2-adapter` is omitted, the whole store is the in-memory pool, and stopping the server is the entire wipe — there is no half-done state to get into between cells. Give it a path and the disk tier comes back, and with it the rule that **wipe and restart are one atomic operation**: stop the running server, delete `$LMCACHE_L2_DIR`, start it again, wait for the port. Never one without the other — the server's L1 index is in memory, so wiping the dir under a live server leaves it serving keys whose backing files are gone, and restarting without wiping carries the previous cell's L2 in. The path is scoped by `BENCH_INSTANCE` so two concurrent stacks cannot wipe each other's. Skipped for `continuum` (that arm uses `LMCACHE_CONFIG_FILE` + `LMCacheConnectorV1`, no separate server). |
...
LMCACHE_L2_DIR=                                         # blank: L1 only, and the restart is the wipe
...
2. `baseline` / `ours`: **stop LMCache → start LMCache → wait for port.** With `LMCACHE_L2_DIR` blank — the default — there is no disk tier and the stop/start is the whole wipe. With a path set, the delete goes between the two and the three are one step, never partly done. `continuum`: skip.
